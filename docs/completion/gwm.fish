...
# Complete add command options and arguments
complete -c gwm -f -n '__fish_seen_subcommand_from add' -l 'create-branch' -s 'b' -d 'Create the branch if it does not exist'
complete -c gwm -f -n '__fish_seen_subcommand_from add' -l 'help' -s 'h' -d 'Print usage information for this command'
complete -c gwm -A -f -n '__fish_seen_subcommand_from add' -a '(gwm --complete add (commandline -ct) (__fish_number_of_cmd_args) 2>/dev/null)' -d 'Branch name'

# Complete switch command options and arguments
complete -c gwm -f -n '__fish_seen_subcommand_from switch' -l 'reconfigure' -s 'r' -d 'Reconfigure the worktree by copying files and running add hooks'
