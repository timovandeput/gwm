...
void main() {
  group('CompletionService', () {
    ...
    group('getCompletions', () {
      ...
      test(
        'returns filtered worktree completions for switch command with partial input',
        () async {
          final mockWorktrees = [
            Worktree(
              name: 'feature-branch',
      ...
      test(
        'returns empty list for add command at position other than 0',
        () async {
          final completions = await completionService.getCompletions(
            command: 'add',
            position: 1,
          ...
          expect(completions, isEmpty);
          verifyNever(() => mockGitClient.listBranches());
        },
      );

      test(
        'returns empty list for switch command at position other than 0',
        () async {
          final completions = await completionService.getCompletions(
            command: 'switch',
            position: 1,
